...
class DropOutLayer(Layer):
    ...
    def forward_pass(self, input_data):
        ...
        print("A", self.activation_output.shape)

        self.d1 = np.random.rand(self.activation_output.shape[0], self.activation_output.shape[1]) < 0.9
        print("d1", self.d1.shape)

        self.activation_output = np.multiply(self.activation_output, self.d1)
        self.activation_output /= 0.9
    ...
    def backward_pass(self, deriv_activation, learning_rate, train_size):
        ...
        deriv_activation = np.multiply(deriv_activation, self.d1)
        deriv_activation /= 0.9
